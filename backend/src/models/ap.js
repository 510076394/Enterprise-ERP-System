  ...
  createInvoice: async (invoiceData, connection = null) => {
    ...
    try {
      ...
      if (invoiceData.status === '已确认') {
        ...
        if (invoiceData.gl_entry) {
          // 调试日志：检查 invoiceData
          logger.info('[AP] 准备创建会计分录，invoiceData:', {
          ...
          // 调试日志：检查 entryData
          logger.info('[AP] entryData准备完成:', entryData);
        ...
        if (bankAccounts.length > 0) {
          ...
          await connection.execute(
            ...
            [
              ...
              paymentItems[0]?.invoice_id || null, // 关联发票ID（取第一个）
              'AP', // 关联发票类型：应付
            ]
          );
      ...
      await connection.execute(
        `UPDATE ap_payments 
         SET status = 'void', 
             ...
             void_reason = ?
         WHERE id = ?`,
        [voidData.voided_by, voidData.void_reason, paymentId]
      );
      ...
      try {
        ...
        if (glEntries.length > 0) {
          for (const glEntry of glEntries) {
            ...
            // 生成冲销凭证编号
            const reversalEntryNumber = `VOID-AP-${payment.payment_number}-${Date.now()}`;
